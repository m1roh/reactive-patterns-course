...
export class CourseComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.lessonsPager.loadFirstPage(this.id)
      .subscribe(
        ...
        () => this.messagesService.error('Could not load first page')
      );
  }
  ...
  previousLessonsPage() {
    this.lessonsPager.previous().subscribe(
      ...
      () => this.messagesService.error('Could not load previous page')
    );
  }
  ...
  nextLessonsPage() {
    this.lessonsPager.next().subscribe(
      ...
      () => this.messagesService.error('Could not load next page')
    );
  }
